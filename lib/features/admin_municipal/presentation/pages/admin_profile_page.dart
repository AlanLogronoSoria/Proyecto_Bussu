...
final municipalNotificationsEnabledProvider = StateProvider<bool>((ref) => true);
...
class AdminProfilePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final notifs = ref.watch(municipalNotificationsEnabledProvider);
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      Center(child: CircleAvatar(radius: 48, backgroundColor: const Color(0xFF001B44), child: Text((user?.fullName ?? 'M')[0].toUpperCase(), style: const TextStyle(fontSize: 32, color: Colors.white, fontWeight: FontWeight.w600, fontFamily: 'Inter')))),
      const SizedBox(height: 12),
      Text(user?.fullName ?? 'Admin Municipal', textAlign: TextAlign.center, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: Color(0xFF001B44), fontFamily: 'Inter')),
      ...
      Center(child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: Colors.purple.withAlpha(20), borderRadius: BorderRadius.circular(8)), child: const Text('Admin Municipal', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.purple, fontFamily: 'Inter')))),
