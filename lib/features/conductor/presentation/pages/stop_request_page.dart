...
class _StopRequestPageState extends ConsumerState<StopRequestPage> {
  ...
  final _requests = [
    {'location': 'Jr. de la Unión 450', 'reason': 'Alta demanda', 'status': 'Pendiente', 'date': '12 may'},
  ...
  Color _sc(String s) => s == 'Aprobada' ? Colors.green : s == 'Rechazada' ? const Color(0xFFBA1A1A) : const Color(0xFFFED000);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        SizedBox(height: 180, child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), boxShadow: const [BoxShadow(color: Color(0x14002F6C), blurRadius: 8)]), clipBehavior: Clip.antiAlias, child: const GoogleMap(initialCameraPosition: CameraPosition(target: LatLng(-12.0464, -77.0428), zoom: 16), myLocationButtonEnabled: true, zoomControlsEnabled: false))),
        ...
        ..._requests.map((r) => Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: const [BoxShadow(color: Color(0x14002F6C), blurRadius: 8)]), child: Row(children: [
          Container(width: 4, height: 48, decoration: BoxDecoration(color: _sc(r['status']!), borderRadius: BorderRadius.circular(2))),
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: _sc(r['status']!).withAlpha(20), borderRadius: BorderRadius.circular(8)), child: Text('${r['status']} · ${r['date']}', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: _sc(r['status']!), fontFamily: 'Inter'))),
        ]))),
      ]),
    );
  }
